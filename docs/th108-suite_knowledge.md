...
- **Known ACK-mute/wedge triggers (environment, characterized 2026-06-09):** the board can drop into **ACK-mute** (stops ACKing 0x32, no disconnect event, typing still works, lighting falls back to the stored firmware effect) from external USB bus events, not just our own streaming. Observed triggers: **powering other hardware on the bus off/on while the PC is awake** — e.g. an audio DAC/switch flip caused a re-enumeration after which a fresh daemon handle-open sat never-ACKed for 8+ minutes; only a physical replug cleared it (replug = real firmware reboot — the TH108 in USB switch position is battery-disconnected). **System sleep entry** also logs as a mute (USB suspend) but is benign and recovers on wake. **Spontaneous episodes** during long sessions self-recover in 7s–10min. Note: mute ≠ visible outage — the board often keeps accepting paint writes it doesn't ACK. The daemon retries every 5s and auto-recovers when ACKs return or on replug. Full characterization lives in the `th108-lighting-protocol` project memory.
