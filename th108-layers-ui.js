    ...
    function buildLayerBody(card,L){
      ...
      if(L.type==='background'){
        ...
        const windowRow=full('<div style="display:flex;flex-direction:column;gap:6px;width:100%">'
          +'<label class="sl" style="margin:0;justify-content:center;color:var(--warn,#e0a200);opacity:.85" title="LOCKED to dry-run in the daemon: window-focus only resolves + LOGS the target window (%TEMP%\\th108-focuswin.log) — it never focuses or flashes. Live window-moving is disabled after it repeatedly closed VSCode; the root cause is still being investigated."><input type="checkbox" class="s-agDryRun" checked disabled> 🔒 Dry-run — LOCKED (window-focus is log-only)</label>'
