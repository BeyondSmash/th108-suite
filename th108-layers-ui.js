      ...
      if(L.type==='background'){
        ...
        let html='<div class="ctl">'+
          ...
          (s.source==='mic' ? sub('Mic Input')+
            ...
            row('Noise Gate','<span class="srange" style="width:100%"><input type="range" class="s-micGate" min="0" max="8" step="0.25" value="'+(s.micGate==null?0:s.micGate)+'" title="Mute the keys until the mic exceeds this absolute level — raise it just above your room/fan noise so background hum stops lighting the board. Scaled 0–8% to match a real mic’s RMS range. 0 = off"><i class="tick" style="left:calc(7px + (100% - 14px)*0.0)"></i></span><span class="val s-micGateV"></span>')+
        ...
        (function(){
          ...
          const micFill=c('.s-micMeterFill'), micGateEl=c('.s-micMeterGate'), MIC_SCALE=0.08;   // meter tops at 8% — a real mic's RMS rarely exceeds that, so the full bar is usable (was 0.6 → signal looked tiny)
