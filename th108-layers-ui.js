  ...
  function create(opts){
    ...
    function buildLayerBody(card,L){
      ...
      if(L.type==='background'){
        ...
        const windowRow=full('<div style="display:flex;flex-direction:column;align-items:center;gap:6px;width:100%">'
          ...
          +'<div class="s-agWinColorRow" style="display:none;align-items:center;justify-content:center;gap:8px;margin-top:2px" title="Give the picked project its OWN outline color. Focusing any window of that project flashes this color instead of the default below."><span style="font-size:12px;color:var(--muted)">Color (override) for <b class="s-agWinColorName" style="color:var(--text,#e6edf3)"></b></span><input type="color" class="s-agWinColor" style="width:34px;height:24px;padding:0;border:none;background:none;cursor:pointer"><button type="button" class="sreset s-agWinColorReset" title="Clear this project’s color — fall back to the default outline color">↺</button></div>'
        ...
        function setAgSymbol(a){ if(!agSymbol) return; const ph=agPhase(a);
          ...
          if(sig===_symSig) return; _symSig=sig;   // rebuild only on a real phase change — re-setting innerHTML every poll otherwise restarts the spinner animation
