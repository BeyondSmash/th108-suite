    ...
    function buildLayerBody(card,L){
      ...
      if(L.type==='background'){
        ...
        if(style==='bars'){ const bt=s.barTip||'off', bf=s.barFill||'solid', bc=s.barColor||'bassTreble', blo=s.barLayout||'standard', bdr=s.barDrive||'spectrum', bsp=!!s.barSpread;
          ...
          row('Density','<span class="srange" style="width:100%"><input type="range" class="s-waveDensity" min="0" max="100" value="'+(s.waveDensity==null?50:s.waveDensity)+'" title="How many waves fit across the board — higher = denser/finer. Capped for readability; the wanted excess turns into faster scroll instead of cramming smaller waves"><i class="tick" style="left:calc(7px + (100% - 14px)*0.5)"></i></span><span class="val s-waveDensityV"></span>')+
